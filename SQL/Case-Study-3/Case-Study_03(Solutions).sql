...
select a.customer_id,a.start_date,b.plan_name,b.price
from (select * from foodie_fi.subscriptions where customer_id in (select * from temp_cte)) a
...
select month(start_date) as month,count(distinct customer_id) as customer
...
select 
sum(case when plan_id= 0 and plan_id2=4 then 1 else 0 end) as customer_count,
...
count(distinct customer_id)*100/(select count(customer_id) from temp_cte) as perc
from temp_cte
group by plan_name;
